...
exposed port with `docker ps`:
        ...
        docker run --env-file .env --P devrel-studio
        docker ps
...
shell):
        ...
        docker run --env-file .env devrel-studio npm run shell

To run a Django shell inside the container:

        docker run --env-file .env -t devrel-studio
...
In Heroku dashboard, create a new app named `devrel-studio`.
Initialize Heroku remote in the repository:

        heroku git:remote -a devrel-studio
